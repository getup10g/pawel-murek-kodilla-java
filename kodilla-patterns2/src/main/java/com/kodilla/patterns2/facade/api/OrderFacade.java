...
public class OrderFacade {
    ...
    public void processOrder(final OrderDto order, final Long userId) throws OrderProcessingException {
        ...
        try {
            ...
            LOGGER.info("Order is reade to submit");
            if (!shopService.submitOrder(orderId)) {
                LOGGER.error(OrderProcessingException.ERR_SUMBITTING_ERROR);
                wasError = true;
                throw new OrderProcessingException(OrderProcessingException.ERR_SUMBITTING_ERROR);
            }
        } finally {
            if(wasError) {
